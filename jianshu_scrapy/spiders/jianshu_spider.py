...
class jianshu_spider(Spider):
    ...
    def parse_author(self, response):
        ...
        if header_part:
            try:
                ...
                # 个人介绍
                author_note = \
                    response.xpath('//div[@class="description"]/div[@class="js-intro"]').xpath('string(.)').extract()[0]
                logging.info(
                    'Author: %s,\n Following: %s, \nFollowers: %s, \nArticle: %s, \nWords: %s, \nLike: %s, \nFollowing_Url: %s, \nFollower_url: %s, \nArticle_url: %s' % (
    ...
    def parse_articles(self, response):
        ...
        if '/u/' in response.url and '?order' in response.url:
            ...
            try:
                ...
                author = \
                response.xpath('//div[@class="main-top"]/div[@class="title"]/a[@class="name"]/text()').extract()[0]
                author_id = \
                    response.xpath('//div[@class="main-top"]/div[@class="title"]/a[@class="name"]//@href').extract()[0]
    ...
    def start_request_follower(self):
        ...
        if parsing_count < 3 and need_parsing_count > 0:
    ...
    def start_request_author(self):
        ...
        if parsingCount < 3 and needParsingCount > 0:
